...
<div class="inner">
	<div class="title">
		<h2>
			<span>${userContent.userNickName}</span>님의 MYPAGE
		</h2>
	</div>
	<hr>
	...
	<button class="btn-yellow" name="btn-fixUser" id="btn-fixUser"
    ...
    <%=linkedAccounts.getOrDefault("naver", false) ? "pointer-events: none; opacity: 0.5;" : ""%>">
		...
		%>
		<p style="font-size: 12px; color: #555; margin-top: 10px;">⚠️
			회원가입한 이메일을 사용하는 계정을 연동해주세요. 연동시 기존 로그인으로 접속됩니다.</p>
	</div>
	...
	<div class="contentInfoBox">
		<c:choose>
			<c:when test="${empty userInfoContent}">
				...
				<button name="btn-makeInfo" id="btn-makeInfo"
					onclick="location.href='${ctx}/userInfo.do'">인포만들기</button>
			</c:when>
			<c:otherwise>
				<table class="infoTable">
					<tr>
						<td>나의 식사량</td>
						<td><span class="mealSize">${userInfoMealSize}</span> 인분의 식사를
							해요</td>
					</tr>
					<tr>
						<td>나의 선호 TAG</td>
						<td><c:forEach var="info" items="${totalInfo}">
								<span class="infoItem">${info}</span>
							</c:forEach></td>
					</tr>
				</table>
				...
				<form id="userInfoFixForm" action="${ctx}/userInfoFix.do"
					...
					<button type="submit" class="btn-yellow" name="btn-makeInfo"
						id="btn-makeInfo">인포수정하기</button>
				</form>

			</c:otherwise>
		</c:choose>
	</div>
	<button class="btn-grey" name="btn-userDel" id="btn-userDel"
